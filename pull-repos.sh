...
fetch_repo() {
    ...
    if [ -d "$dest/.git" ]; then
        ...
        git clone --depth 1 --branch "$ref" --recurse-submodules "$url" "$dest" 2>/dev/null \
            || { git clone --recurse-submodules "$url" "$dest" && git -C "$dest" checkout -q "$ref" && git -C "$dest" submodule update --init --recursive; }
    fi
}
...
for repo in "${REPOS[@]}"; do
    ...
    fetch_repo "$repo" "$ORG/$repo.git" "$ref"
done

fetch_repo "web-next" "$WEB_NEXT_URL" "$WEB_NEXT_REF_DEFAULT"
