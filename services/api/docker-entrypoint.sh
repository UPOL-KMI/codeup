...
ENVSUBST_VARS='$API_ADDRESS $API_INTERNAL_ADDRESS $WEBAPP_ADDRESS $JWT_SECRET $TOKEN_COOKIE_PREFIX $BROKER_ADDRESS $BROKER_AUTH_USER $BROKER_AUTH_PASSWORD $MONITOR_ADDRESS $WORKER_FILES_AUTH_USER $WORKER_FILES_AUTH_PASSWORD $LOCAL_REGISTRATION_ENABLED $MAIL_FROM $ADMIN_EMAIL $DB_HOST $DB_USER $DB_PASSWORD $DB_NAME $SMTP_HOST $SMTP_CLIENT_HOST $SMTP_PORT $SMTP_USER $SMTP_PASSWORD $SMTP_SECURE $RECODEX_TITLE'
